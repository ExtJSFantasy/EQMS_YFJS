...
Ext.define('UX.TouchCalendarSimpleEvents', {
	...
	config: {
		...
		eventTpl: [
			...
			'<div class="{eventDotCls} {[values.color == \'\' ?  \'gray\':(values.color == \'green\' ? \'green\' : (values.color == \'red\' ? \'red\' : (values.color == \'purple\' ? \'purple\':(values.color == \'yellow\' ? \'yellow\': \'\'))))]}" ></div>'				
		].join('')
	},
	...
	refreshEvent: function(store, records){
		...
		Ext.each(records, function(record){
			...
			//console.log(Ext.Date.clearTime(record.get(startField), true));
			if(!record.data.StartDate) 
				record.data.StartDate = Ext.Date.clearTime(record.get(startField), true);
			if(!record.data.EndDate) 
				record.data.EndDate = Ext.Date.clearTime(record.get(endField), true);
			var s = Ext.clone(record.data.StartDate),
				e = record.data.EndDate;
	...
	refreshOneDate: function(date){
		...
		if (cell) {
			...
			store.data.each(function(record){
				//console.log("222222",record.data.StartDate);
				if(!record.data.StartDate) 
					record.data.StartDate = Ext.Date.clearTime(record.get(startField), true);
				if(!record.data.EndDate) 
					record.data.EndDate = Ext.Date.clearTime(record.get(endField), true);
					//console.log(record.data.StartDate - date);
				if((record.data.StartDate - date <= 0) && (record.data.EndDate - date >= 0)) {
					found = record;
